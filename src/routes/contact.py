...
@router.get("/upcoming_birthdays", response_model=List[ContactSchema])
async def get_upcoming_birthdays(
    ...
    contacts = await rep_contacts.upcoming_birthdays(days, skip, limit, db)
    if not contacts:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def create_contact(data: ContactSchema, db: Session = Depends(get_db)):
    return await rep_contacts.craete_contact(data, db)
...
async def update_contact(
    ...
    contact = await rep_contacts.update_contact(contact_id, data, db)
    if contact is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Contact id = {contact_id} is not found."
